...
const Bottom = ({
    ...
    const loadAvatars = async () => {
      ...
      await Promise.all(
        team.map(async (player) => {
          if (player.avatar) {
            try {
              const res = await axios.get(`${BACKEND_URL}/avatar/${player.avatar}`);
              newMap[player.avatar] = res.data.avatar.imageUrl;
            } catch (err) {
              console.error(`Failed to load avatar for ${player.name}`, err);
              newMap[player.avatar] = "/default-avatar.png";
            }
          }
        })
      );
  ...
  return (
    ...
    >
      ...
      {/* video containers */}
      <div className="mt-4 flex gap-4 justify-center pb-4">
        <button
      ...
      <div className="relative flex items-center justify-center mt-4  mr-2">
        {
          showTeam &&
          <div className="absolute bottom-16 bg-[#6c75b5] w-[250px] rounded-lg  max-w-[250px] max-h-[300px] overflow-y-scroll" >

            {
              team.map((player) => {
                const avatarUrl = avatarMap[player.avatar];
                ...
                return <div className="flex items-center justify-between  w-full px-4 py-2">
                  <img
                    ...
                    src={avatarUrl}
                    // src={`https://imgs.search.brave.com/LxndcNWLgB_8KkJr35I0tLhUbeEtHUULu8wrsndXqK8/rs:fit:500:0:0:0/g:ce/aHR0cHM6Ly90NC5m/dGNkbi5uZXQvanBn/LzA4LzYxLzkxLzU3/LzM2MF9GXzg2MTkx/NTc4N19tTWF2UDlj/TWZQOWVXVGJleDY3/cDRMOWd1RkVVSFJD/Ry5qcGc`}
                    alt=""
                  />
                  <div className="flex flex-col  justify-center ">
                    <span className="flex gap-2 ">{player.name} {user.username === player.email && <p className="text-white/40">(You)</p>}</span>
                    <span>{player.email}</span>
                  </div>
                  <span className="text-green-500 rounded-full w-2 h-2 bg-green-500 flex items-center justify-center">
                  </span>
                </div>
              })
            }
        ...
        <button className="cursor-pointer flex justify-center items-center gap-2 p-2 rounded-md bg-[#757ec5] mb-4" onClick={() => setShowTeam(prev => !prev)}> {showTeam ? <FaArrowDown />: <FaArrowUp />}   Live: {team.length}</button>
      </div>
        <button
